function output = RotateAround(image, pointY, pointX, angle)
output = image;
if abs(angle)<eps
	return;
end
...
if(size(output,3)>1)
    output(:,:,2) = rotateAround1(image(:,:,2), pointY, pointX, angle, ...
                                  method);
    output(:,:,3) = rotateAround1(image(:,:,3), pointY, pointX, angle, ...
                                  method);
end


function output=rotateAround1(image, pointY, pointX, angle, varargin)
