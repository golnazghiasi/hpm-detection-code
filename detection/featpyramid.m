...
%maxsize = max([model.maxsize{:}]);
maxsize = model.feature_map_padding;
padx = max(maxsize + 1, 0);
pady = max(maxsize + 1, 0);
...
for i = 1:length(pyra.feat)
    % Adds 1 to padding because feature generation deletes a 1-cell
